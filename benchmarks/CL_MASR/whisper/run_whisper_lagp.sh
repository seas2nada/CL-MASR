...
CONFIG_FILE="hparams/train_lagp.yaml"
# Check for data folder arguments
if [ -z "$1" ]; then
    echo "Usage: $0 <path-to-data-folder> <gpu>"
    echo "Example: $0 /path/to/CL-MASR/data 0"
    exit 1
fi
DATA_FOLDER="$1"
GPU="$2"
...
echo "Running LAGP with $WHISPER_VARIANT using data from $DATA_FOLDER..."
CUDA_VISIBLE_DEVICES=$GPU python train_lagp.py "$CONFIG_FILE" --whisper_variant "$WHISPER_VARIANT" --data_folder "$DATA_FOLDER"