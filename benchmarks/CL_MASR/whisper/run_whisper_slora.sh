...
CONFIG_FILE="hparams/train_slora.yaml"
...
echo "Running Shared LoRA training with $WHISPER_VARIANT using data from $DATA_FOLDER..."
CUDA_VISIBLE_DEVICES=$GPU python train_slora.py "$CONFIG_FILE" --whisper_variant "$WHISPER_VARIANT" --data_folder "$DATA_FOLDER"
