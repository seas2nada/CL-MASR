...
CONFIG_FILE="hparams/train_gamlora.yaml"
...
echo "Running GAMLoRA training with $WHISPER_VARIANT using data from $DATA_FOLDER..."
CUDA_VISIBLE_DEVICES=$GPU python train_gamlora.py "$CONFIG_FILE" --whisper_variant "$WHISPER_VARIANT" --data_folder "$DATA_FOLDER"
