...
CONFIG_FILE="hparams/train_bitfit.yaml"
...
echo "Running BitFit training with $WHISPER_VARIANT using data from $DATA_FOLDER..."
CUDA_VISIBLE_DEVICES=$GPU python train_bitfit.py "$CONFIG_FILE" --whisper_variant "$WHISPER_VARIANT" --data_folder "$DATA_FOLDER"
