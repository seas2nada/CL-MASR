...
def prepare_common_voice(
    ...
    # Use a lock file to prevent race conditions when running multiple processes concurrently
    os.makedirs(data_folder, exist_ok=True)
    lock_file = os.path.join(data_folder, ".prepare.lock")
    with open(lock_file, "w") as f_lock:
        _LOGGER.info(f"Acquiring lock on {lock_file}...")
        ...
        try:
            ...
            _LOGGER.info(f"Releasing lock on {lock_file}...")
...
def compute_clip_durations(locale_folder: "str") -> "None":
    ...
    for split in _SPLITS:
        ...
        if os.path.exists(output_tsv_file):
            ...
            continue
        with open(input_tsv_file, encoding="utf-8", errors="replace") as fr, open(
...
def merge_tsv_files(
    ...
    with open(output_tsv_file, "w", encoding="utf-8") as fw:
        ...
        for input_tsv_file in input_tsv_files:
            ...
            with open(input_tsv_file, encoding="utf-8", errors="replace") as fr:
